...
import { useDayTripPackages } from "@/hooks/usePackages";
import { useLanguage } from "@/i18n/LanguageContext";

const DayTripDetail = () => {
  const { slug } = useParams();
  const { data: packages = [] } = useDayTripPackages();
  const { t } = useLanguage();
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground mb-4">Tour không tồn tại</h1>
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="bg-red-500 text-white p-4 text-center">
        Tour loaded: {packageData.name} | Slug: {slug}
      </div>
      
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-4">{packageData.name}</h1>
        <p className="text-lg text-gray-600 mb-4">No description available</p>
